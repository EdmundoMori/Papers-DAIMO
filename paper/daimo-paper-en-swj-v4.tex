...
\begin{abstract}
Inter-organisational deployment of artificial-intelligence models requires coherent semantic treatment of five tasks that remain fragmented across distinct vocabularies: publishing a model as a catalogue asset, selecting it under a usage policy, invoking it through a typed service, tracing each execution, and comparing models under reproducible evaluation conditions. MLDCAT-AP describes the model; DCAT publishes it; ODRL expresses policy; PROV-O records provenance; and the \emph{Dataspace Protocol} (DSP) negotiates contractual exchange. None of them articulates all five tasks as a single chain.
...
Figure~\ref{fig:ontology-global} provides a functional reading of the complete ontology. It groups bridge classes and reused resources by their role in the publication, invocation, authorisation, evidence, and evaluation chain. This organisation helps read DAIMO as a relational profile rather than as a flat list of terms.
...
\begin{tikzpicture}[node distance=0.25cm]
  \node[groupbox, fill=daimoBridgeFill!24, text width=0.88\columnwidth, align=left,
    ...
    {\textbf{Publication and governance}\\
    \code{AIAssetOffering}; \code{ParticipantRole} and role subclasses.\\
    Main reuse: \code{dcat:CatalogRecord} and \code{odrl:Offer}.};

  \node[groupbox, fill=daimoBridgeFill!24, text width=0.88\columnwidth, align=left,
    font=\sffamily\scriptsize, below=of g1] (g2)
    {\textbf{Deployment and invocation}\\
    \code{ModelDeployment}; \code{IOContract}.\\
    Main reuse: \code{it6:MachineLearningModel} and \code{dcat:DataService}.};

  \node[groupbox, fill=daimoBridgeFill!24, text width=0.88\columnwidth, align=left,
    font=\sffamily\scriptsize, below=of g2] (g3)
    {\textbf{Authorised execution and outputs}\\
    \code{ExecutionAuthorization}; \code{DerivedArtifact}.\\
    Main reuse: \code{odrl:Agreement} and \code{it6:Run}.};
...
\end{tikzpicture}
\caption{Global functional view of DAIMO by conceptual groupings.}
\label{fig:ontology-global}
\end{figure}

Figure~\ref{fig:classes} complements the global view at the axiomatic level. The upper panel separates DAIMO bridge classes and reused classes connected by \code{rdfs:subClassOf}; the lower panel retains only the main operational relations that articulate the profile.
...
The scenario introduced in §\ref{sec:scenario} is instantiated on DAIMO as a controlled demonstrator. The resulting graph contains 225 data triples that exercise every native class at least once and over which the twenty-three competency queries reported in §\ref{sec:eval-cqs} are executed. All organisations, model names, dataset versions, metrics, and endpoints are anonymised or synthetic. The demonstrator is not a deployment study, a gold standard, or a validation against real model catalogues; its function is to examine representational adequacy and make the validation artefacts executable.
...
\end{tikzpicture}
\caption{Phase flow of the multi-participant demonstrator.}
\label{fig:scenario}
\end{figure}
...
Extensibility & The core remains minimal and SemVer-versioned; extensions are expected as complementary profiles, for example \code{daimo-discovery}, without absorbing ranking or runtime-execution logic. & Strong artefact-level coverage for controlled evolution. New concepts must justify that no reused vocabulary already provides them. \\
Modularity & Core, alignments, shapes, examples, queries, negative tests, and reports are separate artefacts; each module can be executed or reviewed independently. & Strong artefact-level coverage. Modular separation reduces the risk of breaking the core when external profiles are added. \\
Documentation & Each term includes SKOS labels and definitions; supplementary documentation traces classes, properties, CQs, invariants, and reproduction commands. & Strong artefact-level coverage for local review and reproduction. Public documentation quality should remain synchronised with future profile versions. \\
\bottomrule
\end{tabular}
\end{table*}

The benchmark is presented as bounded tractability evidence, not as operational performance proof. Table~\ref{tab:scalability} preserves the reported times and counts for 100 and 1,000 synthetic units and shows that SHACL dominates validation cost.
