...
\begin{abstract}
\textbf{Purpose:} AI models are increasingly published, negotiated, deployed, invoked, monitored, and evaluated across organisational boundaries. Existing vocabularies describe important fragments of this lifecycle, but they do not connect catalog publication, policy, deployment, execution provenance, audit evidence, and evaluation context into one auditable governance chain.
\textbf{Methods:} We present DAIMO, the Dataspace AI Model Ontology, an OWL~2~DL integration profile developed with the Linked Open Terms methodology. DAIMO organises established Semantic Web vocabularies around the missing governance relations between catalogues, policies, deployments, executions, provenance records, and evaluations.
\textbf{Results:} The artefact is evaluated through an illustrative multi-participant dataspace scenario, OWL consistency and entailment verification, SHACL structural and cross-class validation, a negative test bench, and twenty-three competency questions executed as SPARQL queries.
\textbf{Conclusions:} DAIMO provides a reusable semantic bridge for governed AI-model assets in dataspaces, making publication, discovery, invocation, traceability, and evaluation queryable and validatable as one coherent lifecycle.
\end{abstract}
...
\midrule
Publication & Is this model a governed asset in a particular catalog context? & Governed offering record linking model, provider, catalog context, and applicable policy. & CQs R1--R5, G1 \\
...
The implementation is decomposed into three Turtle modules, each serving a distinct adoption need. The \textbf{core module} (\code{daimo-core.ttl}) contains the fourteen DAIMO classes, twenty-nine object properties, eight datatype properties, the global disjointness axioms, and the property characterisations (functional, asymmetric, inverse) that capture TBox-level type invariants. The \textbf{alignment module} (\code{alignment.ttl}) encapsulates eight external \code{rdfs:subClassOf} declarations and five external \code{rdfs:subPropertyOf} declarations that connect DAIMO to reused vocabularies, plus minimal declarations of external terms with \code{rdfs:seeAlso} pointers to their source specifications. Across the combined package there are six DAIMO subproperties to external vocabularies because \code{hasOffering}~$\sqsubseteq$~\code{foaf:isPrimaryTopicOf} is declared in the core as the inverse-side accessor of \code{offersModel}. The \textbf{SHACL module} (\code{daimo-shapes.ttl}) contributes eighteen \code{sh:NodeShape}s: nine structural shapes, three conformance shapes over reused classes, and six cross-class invariants expressed with \code{sh:SPARQLConstraint}. Because the SHACL module is declared as an independent \code{owl:Ontology} with its own \code{owl:versionIRI}, the validation layer can evolve without forcing a new logical vocabulary release.

\begin{table*}[t]
\scriptsize\sf\centering
\caption{DAIMO technical implementation profile. The table maps common implementation requirements to existing package artefacts without introducing additional modules or classes.\label{tab:implementation-profile}}
\begin{tabular}{@{}p{3.0cm}p{7.0cm}p{4.8cm}@{}}
\toprule
\textbf{Pillar} & \textbf{DAIMO implementation} & \textbf{Scope decision} \\
\midrule
Language, serialisation, and IRIs & OWL~2~DL core in Turtle; RDF/XML, JSON-LD, and N-Triples serialisations prepared for publication; canonical namespace \code{https://w3id.org/pionera/daimo}. & The \code{w3id} redirect and DOI are final FAIR-publication steps; local reproducibility does not depend on them. \\
...
Critical relations & \code{it6:trainedOn} is reused; formats and schemas are expressed through \code{IOContract}; deployment reaches endpoints through \code{exposedAs} and \code{dcat:DataService}; regulatory conformance can be expressed through \code{dct:conformsTo} or dataspace-specific ODRL/SHACL profiles. & No duplicate properties such as \code{trainedOn}, \code{deployedIn}, or \code{compliesWith} are added when a standard or profileable path already exists. \\
Validation and reasoning & Structural SHACL, SHACL over reused classes, six SHACL-SPARQL invariants, HermiT, OWL-RL, OOPS!, negative tests, and 23 SPARQL queries. & No universal metric such as accuracy is required; mandatory metrics depend on the evaluation context and the CQ being exercised. \\
Storage and access & The RDF package can be loaded into standard triplestores and exposed through SPARQL; the published queries in \code{daimo/queries/queries.md} demonstrate the access pattern. & The article does not claim a production GraphDB/Fuseki endpoint; it claims technical compatibility with such deployment. \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:metrics} summarises the size and expressivity of the artefact, computed from the ontology files and validation reports. The numbers let a reader compare DAIMO with previously published ontologies in the same family.
...
\begin{table}[t]
\small\sf\centering
\caption{Ontology axiom metrics computed over the DAIMO source modules after the v0.1.6 alignment cleanup.\label{tab:ontometrics}}
...
\code{AIAssetOffering} and \code{ParticipantRole} jointly solve the publication-side gap. MLDCAT-AP describes the model, and DCAT records catalog metadata, but neither expresses the governed act of making a model available to another dataspace participant. The offering is therefore treated as the catalog-facing record of a model in a specific dataspace, scoped by policy, provider, target model, and participant context. \code{ParticipantRole}, with five non-disjoint specialisations, records the role a legal agent plays in relation to an asset and a dataspace context. The two classes jointly answer CQ-R1, CQ-R2, CQ-R5, and CQ-G1.

\medskip\noindent\emph{Execution plane.}
\code{ModelDeployment} and \code{IOContract} cover the invocation-side gap. DCAT supplies catalog-visible services, but it does not say which model a service hosts or which syntactic and authentication expectations a consumer must satisfy to call it. A deployment represents the running or hosted instance of a model on compute infrastructure. It may be exposed through one or several services, and each service may declare an invocation contract covering input/output format, authentication method, and optional schema references. Invocation is not modelled as a direct model-consumer property: it is reconstructed by traversing offering, deployment, service, I/O contract, authorisation, and \code{it6:Run}, avoiding redundant relations such as \code{isInvokedBy}. The profile records where and under which contract the model can be invoked; it does not model container orchestration, the enforcement engine, or the TEE mechanism, which should be declared as infrastructure metadata or specific profiles when a policy requires them. This group answers CQ-E1, CQ-E3, and CQ-G2.
...
\code{SharedEvaluationContext} covers the comparability gap that plain MLDCAT-AP evaluations leave open. MLDCAT-AP records a measurement attached to a model; it does not reify the combination of task, dataset version, evaluation protocol, and random seed that makes two measurements comparable. Metrics are modelled as resources linked to an evaluation and a context, not as fixed attributes of the model: the same model version may exhibit different accuracy, bias, latency, or computational cost under different datasets, protocols, or infrastructures. Two evaluations sharing that context are comparable by construction; evaluations that differ on any coordinate are not. This comparability is operational and minimal: it assumes that the referenced metric has the same definition, unit, and calculation procedure, which metric or protocol profiles can declare when a dataspace requires them. The class answers CQ-V1, CQ-V2, CQ-V3.

\medskip\noindent\emph{Justifying the principal alignments.}\label{sec:alignment-defense}
Two of the eight \code{rdfs:subClassOf} alignments carry more semantic commitment than the others. Plausible alternative modelling choices exist for both, so their justification is made explicit.
...
Governance bridge (G) & G1(1) G2(2) G3(1) G4(1) & 4 & \textbf{G4} (\code{GROUP\_CONCAT} aggregation); G3 directly exercises \code{ExecutionAuthorization} as the DAIMO--ODRL bridge \\
\bottomrule
\end{tabular}
\end{table*}
...
Two fragments illustrate the pattern. Listing~\ref{lst:cq-g3} shows CQ-G3: the query retrieves the authorisation that covers a concrete run, the authorised grantee, and the expiry time. The alignment \code{ExecutionAuthorization}~$\sqsubseteq$~\code{odrl:Agreement} makes the resource interpretable as an ODRL agreement without duplicating contract properties in the query. Listing~\ref{lst:cq-v3} shows CQ-V3: two evaluations are compared only because they share the same \code{SharedEvaluationContext}; ordering by metric value is meaningful inside that context rather than as a global model ranking. Both queries run over the OWL-RL-materialised closure of the demonstrator; the full query set is released in \code{daimo/queries/queries.md}.
...
\begin{table*}[t]
\scriptsize\sf\centering
\caption{Coverage of DAIMO non-functional requirements. The status column separates evidence demonstrated in the artefact from operational tasks still required for the final public release.\label{tab:nfr-coverage}}
\begin{tabular}{@{}p{2.7cm}p{8.0cm}p{4.0cm}@{}}
\toprule
\textbf{Requirement} & \textbf{Reproducible evidence} & \textbf{Status and residual risk} \\
\midrule
Logical consistency & HermiT reports consistency and zero unsatisfiable classes; OWL-RL materialises 1,170 triples without \code{owl:Nothing}; entailment verification inspects fourteen native classes with zero warnings. & Strong coverage. Consistency is complemented with SHACL negative tests so constraints are not merely trivially satisfiable. \\
Interoperability & The profile uses RDF/OWL~2~DL, SHACL, and SPARQL; it reuses DCAT, MLDCAT-AP, ODRL, PROV-O, FOAF, SKOS, and SPDX through external IRIs and axiomatic alignments. & Strong artefact-level coverage. FAIR publication tasks remain: \code{w3id}, Zenodo DOI, and final WIDOCO regeneration. \\
Scalability & \code{scalability\_benchmark.py} generates conforming synthetic graphs of 100 and 1,000 exchange units; both conform to SHACL and answer the control queries. & Measured bounded coverage. SHACL is the dominant cost; no production-throughput or 10,000-unit result is claimed unless executed. \\
Extensibility & The core remains minimal and SemVer-versioned; extensions are expected as complementary profiles, for example \code{daimo-discovery}, without absorbing ranking or runtime-execution logic. & Strong coverage for controlled evolution. New concepts must justify that no reused vocabulary already provides them. \\
Modularity & Core, alignments, shapes, examples, queries, negative tests, and reports are separate artefacts; each module can be executed or reviewed independently. & Strong coverage. Modular separation reduces the risk of breaking the core when external profiles are added. \\
Documentation & Terms include labels and SKOS definitions; \code{ONTOLOGY-REFERENCE.md}, \code{VALIDATION-MATRIX.md}, and \code{REPRODUCIBILITY.md} trace classes, properties, CQs, invariants, and commands. & Strong local coverage. Public WIDOCO documentation and ORCID/DOI metadata must be finalised before submission. \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\small\sf\centering
\caption{Bounded synthetic scalability benchmark. Each unit contains one offering, model, ODRL policy, deployment, service, I/O contract, authorisation, run, derived artefact, audit evidence, evaluation, and provenance bundle. Full report: \code{reports/scalability-benchmark.md}.\label{tab:scalability}}
...
The central contribution of DAIMO is the articulation of five lifecycle relations that are usually documented separately: publication, policy, execution, provenance, and evaluation. This is a semantic and governance contribution: DAIMO does not standardise model weights, internal architectures, or inference mechanisms, but makes the relations among model metadata, policies, deployments, runs, evidence, and evaluation contexts queryable and validatable. Relative to recent domain ontologies such as RePlanIT~\cite{kurteva}, GloSIS~\cite{palma}, and Woods et al.~\cite{woods}, DAIMO shares an engineering ethos of reuse, executable implementation, and CQ-based evaluation, but applies it to the catalogue-dataspace boundary. Relative to earlier machine-learning lifecycle ontologies, DAIMO shares the application domain but adds the governance layer needed for dataspace exchange. Three design features support this contribution. First, entailment verification over the OWL-RL closure complements consistency reasoning by detecting modelling-inappropriate alignments that a DL reasoner may accept without contradiction. Second, cross-class SHACL-SPARQL invariants are paired with a negative test bench, so every invariant is evaluated against a case built to violate it as well as against the conforming graph. Third, reuse is formalised rather than asserted informally: external commitments are represented as explicit alignments, while three tempting PROV-O alignments are deliberately excluded because they would introduce unintended typings.

Three lines of argument bound the limitations. The first concerns \emph{semantic depth}. A transversal limitation is domain dynamism: new architectures, deployment formats, metrics, and evaluation practices evolve faster than ontology-maintenance cycles, so they should enter through versioning and complementary profiles rather than rushed core extensions. DAIMO covers discovery of AI models as governed assets: CQs D1--D4 locate models by task, domain, policy, endpoint, and evaluation threshold. It does not claim to be a catalogue of catalogues or a federated-query engine, nor does it by itself cover discovery of data models, schemas, or sector vocabularies. The I/O contract captures format and authentication method, with the latter controlled through \code{sh:in}. Optional \code{inputSchema}, \code{outputSchema}, and \code{dct:conformsTo}-based profiles can point to external schemas or standards, but fine-grained semantic compatibility between model and consumer schemas remains outside the core and should be addressed by complementary profiles, such as \code{daimo-discovery}, or dataspace-specific shapes. Similarly, DAIMO describes authorised and auditable executions; it does not implement federated enforcement. Connectors, neutral nodes, containers, and TEE mechanisms belong to the platform layer and can be reflected through metadata or profiles, not as universal axioms. The shared evaluation context reifies task-dataset-version-protocol-seed for single-metric comparisons; heterogeneous benchmarking protocols, composite metric sets, and mathematical equivalence of indicators computed by different procedures remain open. Bias, fairness, security, or interpretability metrics can be represented as evaluation measures when they exist and are contextualised; DAIMO does not define a universal theory of those concepts. Protected attributes, mitigation techniques, and bias-audit workflows belong to compliance or domain profiles. The role model folds role type into role assignment, which OntoClean recommends splitting~\cite{ontoclean}. MLDCAT-AP makes a comparable compromise in its registered-user relation, which places fine-grained contextualisation in the future-refinement horizon, not at the level of a present design error.
...
A practical variant of this issue concerns data sovereignty: restrictions may need to propagate from a training dataset to a model or to its derived artefacts. DAIMO does not axiomatise this propagation as a universal rule, because a model does not automatically inherit every legal or territorial restriction of its source data. The answer depends on licence, contract, jurisdiction, memorisation risk, training technique, and agreed policy. What DAIMO provides is the structure for representing and auditing such propagation when an organisation requires it: \code{it6:trainedOn} and \code{daimo:contextDataset} connect model, evaluation, and dataset; ODRL expresses usage restrictions; \code{ExecutionAuthorization} binds those conditions to concrete runs; and audit evidence together with the cross-participant provenance record make it possible to reconstruct whether the exchange respected the policy. Stricter propagation rules should be implemented as dataspace-specific SHACL or ODRL profiles, not as universal axioms in the core.

The third line concerns \emph{reproducibility conditions and external validation}. Reported numerical results depend on a specific reasoning profile: \code{inference=rdfs} mode in pyshacl, necessary for implied subproperties to materialise before the conformance shapes are evaluated, and the OWL-RL closure materialised before the competency queries. This dependency is documented but is a condition a consumer must know to reproduce exact results. Expert-based validation is not part of the evidence reported here; unlike RePlanIT's interface evaluation and FATO's explicit use of domain experts, the present article reports executable validation rather than practitioner validation. Before final submission, this limitation should be closed with a lightweight qualitative review by three human experts: one ontology/Semantic Web expert, one dataspace or EDC expert, and one ML/MLOps expert. To prevent expert validation from becoming an informal review, materials should be separated by audience: a normalised glossary, competency questions, conceptual diagram, and anonymised scenario for domain experts; OWL excerpts, alignments, and SHACL shapes for the Semantic Web reviewer. Expert-review questions do not replace LOT competency questions: CQs are verified through SPARQL over the demonstration graph, whereas expert review assesses sufficiency, clarity, alignment with dataspace practice, and usefulness for MLOps or compliance. Acceptance should not require unanimity or nominal 100\% coverage; it should require every observation to be traced as an accepted change, a rejected change with justification, out of scope, or future work, together with its impact on classes, properties, CQs, or constraints. The review protocol is fixed in the repository and records each expert profile, observation, action taken or rejected, and impact on DAIMO. A compliance reviewer is recommended when the submitted version keeps explicit claims about the AI Act, bias, or responsibility.
...
This delimitation also positions DAIMO with respect to discovery applications such as \code{ai-model-discovery}. DAIMO should not absorb ranking, recommendation, federated-execution orchestration, or user-interface logic from a particular application; those capabilities belong to an operational layer that consumes the DAIMO graph. A reasonable extension path is a complementary \code{daimo-discovery} profile for indexing metadata, aggregate popularity signals, observed latency, computational footprint, and sustainability indicators, all attached to evaluation or monitoring contexts rather than inserted into the DAIMO core. In this way, \code{ai-model-discovery} can act as a future application and exploitation bench while DAIMO remains publishable as an integration ontology for governed exchange.

The same rule applies to complementary questions on availability, deep reproducibility, and operational trust. Some are already covered by the present profile because they belong to the catalogue-policy-deployment-execution chain; others belong to the MLDCAT-AP asset layer or to future profiles. Table~\ref{tab:future-profiles} summarises the scope decision. The table does not introduce new ontology modules; it documents which questions are core DAIMO questions and which should evolve as specialised profiles so that the main contribution remains focused before submission.

\begin{table*}[t]
\small\sf\centering
\caption{Coverage of complementary question families and controlled extension path. The criterion is to keep the DAIMO core limited to the catalogue-dataspace bridge and move MLOps, security, or fine-grained compliance detail into specialised profiles.\label{tab:future-profiles}}
\begin{tabular}{@{}p{3.2cm}p{5.9cm}p{6.0cm}@{}}
\toprule
\textbf{Family} & \textbf{Coverage in current DAIMO} & \textbf{Controlled extension path} \\
\midrule
Resource availability & Endpoint through \code{ModelDeployment}, \code{dcat:DataService}, and \code{dcat:endpointURL}; policy and licence through ODRL and \code{dct:license}; downloadable distributions through DCAT when the model is published as a file; provider through DAIMO roles and participant context. \code{dct:publisher} remains available for the catalog or record maintainer where needed. & Keep in the core only the availability required for discovery and invocation. Artefact formats such as ONNX or PyTorch can be expressed with \code{dct:format}/\code{dcat:Distribution}, or with \code{IOContract} when they affect invocation. \\
Technical reproducibility & Training and evaluation datasets through MLDCAT-AP, DCAT, and PROV-O; reproducibility artefacts through \code{it6:Flow}, evidence, checksum, and shared evaluation context. & Hyperparameters, software dependencies, framework versions, GPU type, and memory belong to \code{daimo-reproducibility} or to more detailed reuse of ML-Schema/MEX/MLDCAT-AP, not to new core classes. \\
Model versioning and lineage & DAIMO can represent derivation and revision through PROV-O when the graph declares them, and can audit the run-artefact-authorisation chain. & A lineage profile can add SHACL rules for \code{prov:wasDerivedFrom}, \code{prov:wasRevisionOf}, obsolescence status, and retraining traces without changing the fourteen native classes. \\
MLOps automation & Outside the core. DAIMO defines the semantic contract that a tool can populate, but does not implement connectors or automatic extraction. & Connectors from Hugging Face, MLflow, or model registries can populate DAIMO graphs and provide later application-level validation. \\
Monitoring and drift & Outside the core unless observations are published as contextualised evaluations. & A \code{daimo-monitoring} profile can model drift, degradation, observed latency, computational footprint, or sustainability as temporal metrics or evaluation events. \\
...
DAIMO is prepared as a release package under CC-BY~4.0 (ontology and documentation) and Apache~2.0 (validation scripts). The package is designed to satisfy the FAIR principles~\cite{fair}: \emph{findability} through the persistent \code{w3id.org/pionera/daimo} namespace with per-version IRIs and Dublin Core metadata; \emph{accessibility} through content negotiation across Turtle, RDF/XML, JSON-LD, and N-Triples; \emph{interoperability} through reuse of W3C and community-standard vocabularies and the OWL~2~DL profile; and \emph{reusability} through SKOS definitions and examples per term~\cite{skos}, SemVer versioning (with a deprecation policy based on \code{owl:deprecated} and \code{skos:closeMatch}), WIDOCO documentation~\cite{widoco}, and SHACL shapes that act as a data contract. The artefact-availability appendix lists the deliverable items and their locations, and the full release procedure is documented in \code{DEPLOYMENT.md} so that future maintainers can reproduce the release process. Maintenance follows LOT Phase~4 through bounded change-impact notes: each non-trivial change must state which LOT activities are reopened and how competency questions, SHACL shapes, alignment axioms, instantiated graphs, and interoperability between dataspace nodes are affected. A public continuous-integration workflow is a desirable engineering closure for automatically running RDF syntax checks, HermiT/OWL-RL, SHACL, OOPS!, and negative tests on every change; it is not reported as additional evidence until such workflow is deployed.

% =====================================================================
\section{Conclusions}\label{sec:conclusion}

DAIMO contributes a governance bridge between the AI catalogue and the dataspace: offering, authorisation, deployment, derived artefact, and cross-participant provenance. The four governance-category competency questions make the gap concrete; they cannot be answered with any of the reused vocabularies alone. Entailment verification, a negative SHACL-SPARQL test bench, and the reproducible demonstrator provide complementary evidence for the profile. Future work follows from the bounded limitations: external expert validation, instantiation over a public MLDCAT-AP catalogue, refinement of I/O contracts toward schema and sector-standard compatibility, role-type/role-assignment separation following OntoClean~\cite{ontoclean}, extension of shared evaluation contexts to heterogeneous benchmarking protocols, automatic population from model registries, and complementary profiles for reproducibility, monitoring, security, and compliance. The release package is prepared under CC-BY~4.0 (ontology) and Apache~2.0 (scripts).
...
\midrule
Persistent namespace & \url{https://w3id.org/pionera/daimo}; content negotiation configuration prepared under \code{w3id-redirect/.htaccess}. Status: pending merge in \code{perma-id/w3id.org}. \\
...
Public repository & Source package prepared; final public repository URL must be fixed before submission by executing \code{DEPLOYMENT.md}. \\
Author identification & Author ORCIDs pending confirmation and insertion in the submission version and in \code{CITATION.cff}. \\
Versioned archival (DOI) & Versioned release prepared for Zenodo archival; the final DOI must be inserted in the submission version. \\
...
The reproducibility workflow does not require prior deployment of the persistent URL. Any user with Python~3.12 and the declared dependencies (\code{rdflib}, \code{pyshacl}, \code{owlrl}, \code{owlready2}) can run the five scripts locally and reproduce every report in \code{reports/}. The persistent URL and the DOI are additional FAIR requirements that complement but do not condition reproducibility.
...
\item \cq{CQ-R2}: For a given model, which catalog offering registered it, by which provider, and when? (requires \code{subPropertyOf} chain)
