...
% Float placement: keep body text between floats and avoid stacking
% several tables/figures together without intervening paragraphs.
...
Requirements specification is grounded in a bounded flood-risk prediction scenario, inspired by PIONERA-style dataspace exchanges, which later acts as the case study (?\ref{sec:eval-usecase}). A university research group, acting as model provider, publishes a flood-risk prediction model in a federated catalogue managed by a dataspace platform operator. A municipal consumer needs to invoke the model to support operational decisions after an extreme-weather event, but cannot do so without first discovering it under a policy compatible with public use, verifying that it meets a minimum quality threshold under a shared evaluation context, negotiating an agreement that authorises its concrete executions, and invoking it through an authenticated endpoint. A climate-research evaluator compares the model with a baseline under the same evaluation context, and a compliance-oriented governance actor audits executions and archives the corresponding evidence. The organisations, model versions, endpoints, metrics, and dates used in the paper are illustrative: the case is not presented as a deployed pilot, but as a reproducible demonstrator of representational adequacy at the artefact level.
...
The flood-risk scenario introduced in ?\ref{sec:scenario} is instantiated on DAIMO as a controlled demonstrator. The resulting graph contains 225 data triples that exercise every native class at least once and over which the twenty-three competency queries reported in ?\ref{sec:eval-cqs} are executed. All organisations, model names, dataset versions, metrics, and endpoints are illustrative, so the demonstrator exercises the profile conceptually rather than mirroring any deployed instance. The demonstrator is not a deployment study, a gold standard, or a validation against real model catalogues; its function is to examine representational adequacy and make the validation artefacts executable.

The model provider publishes three governed model offerings in the federated catalogue: two versions of the same flood-risk prediction model and one research-only variant with a stricter policy. The offerings share task and domain descriptions but differ in the permissions attached to them. This part of the scenario exercises the distinction between a model as an AI asset and an offering as a contextual catalogue record with its own usage conditions.

The selected flood-risk model is deployed on a described compute environment and exposed through two service interfaces: a REST endpoint with JSON input and bearer-token authentication, and a gRPC endpoint with protobuf and mutual TLS. The multi-endpoint usage exercises the modelling decision that a single deployment may expose several services and contracts, rather than forcing one endpoint per deployment.

The municipal consumer obtains an execution authorisation with explicit validity dates, invokes the model, and receives a derived prediction artefact. The execution is linked to audit evidence containing an integrity record, signer, and timestamp, so that a governance actor can reconstruct how the artefact was produced and under which authorisation. A cross-participant provenance record aggregates the chain across the provider, consumer, and platform contexts. A climate-research evaluator publishes two synthetic evaluation results under the same benchmark, protocol, and random seed, allowing the two model versions to be compared under shared conditions.
...
\begin{tikzpicture}[node distance=0.23cm]
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{1. Publication} \hfill Model provider\\
    \code{AIAssetOffering}; \code{dcat:Catalog}; \code{odrl:Offer}.};

  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    align=left, fill=white, below=of s1] (s2)
    {\textbf{2. Discovery and selection} \hfill Municipal consumer\\
  ...
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{4. Authorised execution} \hfill Municipal consumer\\
  ...
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{6. Shared evaluation} \hfill Climate-research evaluator\\
...
\midrule
Logical consistency & HermiT reports consistency and zero unsatisfiable classes; OWL-RL materialises 1,170 triples without \code{owl:Nothing}; entailment verification inspects fourteen native classes with zero warnings. & Reproducible artefact-level evidence. Consistency is complemented with SHACL negative tests so constraints are not merely trivially satisfiable. \\
Interoperability & The profile uses RDF/OWL~2~DL, SHACL, and SPARQL; it reuses DCAT, MLDCAT-AP, ODRL, PROV-O, FOAF, SKOS, and SPDX through external IRIs and axiomatic alignments. & Substantial artefact-level evidence. Operational interoperability will depend on effective publication and consumption in third-party infrastructures. \\
...
Extensibility & The core remains minimal and SemVer-versioned; extensions are expected as complementary profiles, for example \code{daimo-discovery}, without absorbing ranking or runtime-execution logic. & Substantial artefact-level evidence for controlled evolution. New concepts must justify that no reused vocabulary already provides them. \\
Modularity & Core, alignments, shapes, examples, queries, negative tests, and reports are separate artefacts; each module can be executed or reviewed independently. & Reproducible artefact-level evidence. Modular separation reduces the risk of breaking the core when external profiles are added. \\
Documentation & Each term includes SKOS labels and definitions; supplementary documentation traces classes, properties, CQs, invariants, and reproduction commands. & Substantial artefact-level evidence for local review and reproduction. Public documentation quality should remain synchronised with future profile versions. \\
\bottomrule
\end{tabular}
\end{table*}
...
The ontology, SHACL shapes, demonstrator graph, SPARQL queries, negative tests, validation reports, and reproduction instructions are organised in a single package under \url{https://github.com/EdmundoMori/Papers-DAIMO} (directory \code{daimo/}). For review, the immutable snapshot \url{https://github.com/EdmundoMori/Papers-DAIMO/releases/tag/v0.1.6-swj-submission} is the authoritative frozen release. The canonical namespace \url{https://w3id.org/pionera/daimo} has a prepared redirect configuration (\code{daimo/w3id-redirect/.htaccess}); registration in \emph{w3id.org} remains pending. A Zenodo archival DOI will be assigned after camera-ready acceptance.
...
\bibitem{mex} Keet, C.M., Fern\'andez, J.D. and Panov, P. (2016) MEX vocabulary: A lightweight interchange format for machine learning experiments. \emph{ISWC}.
...
\bibitem{lot22} Poveda-Villal\'on, M., Fern\'andez-Izquierdo, A., Fern\'andez-L\'opez, M. and Garc\'ia-Castro, R. (2022) LOT: An industrial oriented ontology engineering framework. \emph{Engineering Applications of Artificial Intelligence}, 111:104755.
...
\bibitem{oops14} Poveda-Villal\'on, M., G\'omez-P\'erez, A. and Su\'arez-Figueroa, M.C. (2014) OOPS! (OntOlogy Pitfall Scanner!). \emph{International Journal on Semantic Web and Information Systems}, 10(2):7--34.
