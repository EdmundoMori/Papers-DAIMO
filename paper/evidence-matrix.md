...
|---|---|---|---|
| Ontología núcleo | `daimo/ontology/daimo-core.ttl` | — | 14 clases nativas, 31 `owl:ObjectProperty`, 8 `owl:DatatypeProperty`, 29 `owl:FunctionalProperty`, 7 `owl:AsymmetricProperty`, 5 `owl:inverseOf` |
| Alineamientos | `daimo/ontology/alignment.ttl` | — | 7 `rdfs:subClassOf` (`ExecutionAuthorization ⊑ prov:Entity`, no `odrl:Agreement`); 4 `rdfs:subPropertyOf` en el módulo (`grantedTo` ya no ⊑ `odrl:assignee`; `forService` y `derivedFromAgreement` NO alineados) |
| Shapes SHACL | `daimo/shapes/daimo-shapes.ttl` | — | 22 `sh:NodeShape` (9 completitud + 4 conformidad + 9 invariantes), 9 `sh:SPARQLConstraint`; conformidad vía `sh:targetObjectsOf` (DAIMO-ISSUE-04), no `sh:targetClass` global sobre Offer/Model/Run |
| Grafo positivo | `daimo/examples/flood-risk-scenario.ttl` | — | 233 tripletas de datos; acuerdo y autorización son individuos distintos |
| Grafo negativo | `daimo/tests/negative-examples.ttl` | — | 209 tripletas; INV-1..INV-9 + AUTH-no-agreement + AUTH-bad-agreement |
...
| Reporte negativo | `daimo/reports/negative-test-results.md` | — | 9/9 invariantes + 2 reglas per-class disparadas |
...
| `owl:ObjectProperty` | 31 | grep |
| `owl:DatatypeProperty` | 8 | grep |
| `owl:FunctionalProperty` | 29 | rdflib count of typed properties (a comment mentioning the term inflates grep to 30) |
| `owl:AsymmetricProperty` | 7 | grep |
| Pares `owl:inverseOf` | 5 | grep |
| `rdfs:subClassOf` a vocabularios externos | 7 | grep `alignment.ttl` (`ExecutionAuthorization ⊑ prov:Entity`) |
| `rdfs:subPropertyOf` a vocabularios externos | 4 en `alignment.ttl` (+ `hasOffering ⊑ foaf:isPrimaryTopicOf` en el núcleo) | grep; `grantedTo` ya no ⊑ `odrl:assignee` |
| `sh:NodeShape` totales | 22 | grep `daimo-shapes.ttl` |
| Shapes de completitud | 9 (una por clase DAIMO) | `daimo-shapes.ttl` |
| Shapes de conformidad | 4 (`OfferInDAIMOShape`, `AgreementInDAIMOShape`, `MachineLearningModelInDAIMOShape`, `RunInDAIMOShape`) | `daimo-shapes.ttl` |
| Invariantes `sh:SPARQLConstraint` | 9 (INV-1…INV-9) | `daimo-shapes.ttl` |
| Preguntas de competencia | 23 | `queries/queries.md` |
| Tripletas del grafo positivo | 233 | `validate.py` output |
| Tripletas del grafo negativo | 209 | `negative_test.py` output |
| HermiT consistencia | True, 1.71s, 0 insatisfactibles | `reasoner-report.md` |
| OWL-RL materialización | 850 pre → 2043 post (1193 derivadas), 0 `owl:Nothing` | `reasoner-report.md` |
| Entailment check | 14 clases inspeccionadas, 0 advertencias (incl. no `odrl:Agreement`/`odrl:Policy` sobre `ExecutionAuthorization`) | `reasoner-report.md` |
| OOPS! Críticos / Importantes / Menores | 0 / 0 / 2 | `oops-report.md` (scan anterior a `forService`/`derivedFromAgreement`) |
...
| SHACL conformidad positivo | conforms=True, 233 tripletas | `validation-results.md` |
| SHACL conformidad negativo | conforms=False, 9/9 invariantes + 2 reglas per-class disparadas | `negative-test-results.md` |
...
| `daimo:ExecutionAuthorization` | `prov:Entity` | `alignment.ttl`; deliberadamente **no** ⊑ `odrl:Agreement` (DAIMO-ISSUE-02); el acuerdo se reutiliza como individuo distinto vía `daimo:derivedFromAgreement` |
...
### 2.2 `rdfs:subPropertyOf` (4 en `alignment.ttl`; + `hasOffering` en el núcleo)
...
| `daimo:hasOffering` | `foaf:isPrimaryTopicOf` | Simetría con `offersModel ⊑ foaf:primaryTopic` (declarada en el núcleo) |

### 2.3 Subpropiedades deliberadamente NO declaradas
...
- `daimo:grantedTo` ⊄ `odrl:assignee` — **retirado en DAIMO-ISSUE-02**: el dominio `odrl:Policy` re-tiparía la autorización como política; equivalencia grantee = assignee del acuerdo vía SHACL INV-9
- `daimo:grantedTo` ⊄ `prov:qualifiedAssociation` — tiparía al agente como asociación reificada
- `daimo:derivedFromAgreement` sin superpropiedad externa — se consideró y descartó `prov:wasDerivedFrom` (linaje genérico Entity→Entity que forzaría cada `odrl:Agreement` a `prov:Entity`)
...
| Tabla métricas §4.2 | 14 clases, 31 OP, 8 DP, 29 Func, 7 Asym, 5 Inv, 7 subClassOf, 4+1 subPropertyOf, 9+4 shapes, 9 invariantes | factual | Sección 1 de esta matriz (todos verificados por grep o reporte); DAIMO-ISSUE-02 | verified |
...
| §4.3.5 | `ExecutionAuthorization` ⊑ `prov:Entity` (NO ⊑ `odrl:Agreement`); `derivedFromAgreement` al `odrl:Agreement` distinto | factual | `alignment.ttl` + `daimo-core.ttl` + ejemplo `ex:agreement-municipality-flood-v2` ≠ `ex:authorization-municipality-flood-v2` | verified |
...
| §4.3.9 | `SharedEvaluationContext` reifica tarea, dataset, versión, protocolo y, cuando corresponda, semilla | factual | `daimo-core.ttl`; `randomSeed` 0..1 en `SharedEvaluationContextShape`; `tests/random_seed_test.py` | verified |
...
| §4.4 ¶2 | Defensa de separar `ExecutionAuthorization` del `odrl:Agreement` y de no alinear `derivedFromAgreement` a `prov:wasDerivedFrom` | interpretive | DAIMO-ISSUE-02; `docs/01-DAIMO-DESIGN.md` §5.2; `alignment.ttl` | verified |
| §4.5 ¶1 | Axioma nombrado `daimo:TopLevelKindsDisjointness` | factual | `daimo-core.ttl` (grep `AllDisjointClasses`) | verified |
| §4.5 ¶2 | 29 propiedades funcionales, 7 asimétricas, 5 pares inversos | factual | matriz §1 | verified |
| §4.5 ¶3 | Subpropiedades deliberadamente NO declaradas (incl. `grantedTo` ⊄ `odrl:assignee`, `derivedFromAgreement` ⊄ `prov:wasDerivedFrom`) | interpretive | `docs/01-DAIMO-DESIGN.md` §5.2 + `alignment.ttl` | verified |
...
| §5.1 | Grafo de 233 tripletas; autorización y acuerdo son individuos distintos | factual | `validate.py` output; `flood-risk-scenario.ttl` | verified |
...
| §5.2 ¶1 | HermiT consistente ~1.6s, 0 insatisfactibles | factual | `reasoner-report.md` | verified |
| §5.2 ¶1 | OWL-RL materializa 1193 tripletas (850→2043), 0 `owl:Nothing` | factual | `reasoner-report.md` | verified |
...
| §5.3 ¶2 | 9 invariantes INV-1…INV-9 detalladas | factual | `daimo-shapes.ttl` (grep SPARQLConstraint) | verified |
| §5.3 ¶3 | Listado INV-5 como `sh:SPARQLConstraint` | factual | `daimo-shapes.ttl` líneas de `OfferingPolicyTargetInvariant` | verified |
| §5.3 ¶4 | 9/9 invariantes + 2 reglas per-class disparadas sobre banco negativo | factual | `negative-test-results.md` | verified |
...
| §5.5 | Reuso verificado: 7 subClassOf, 4 subPropertyOf en `alignment.ttl` (`grantedTo` ya no ⊑ `odrl:assignee`) | factual | sección §2 de esta matriz | verified |
