...
% Float placement tuning: avoid stacking several floats on one page without
% intervening text. At most one full-width float at the top of a page, and
% keep enough body text on every page so tables/figures are separated.
...
El bloque superior de la Tabla~\ref{tab:mldcatap-delta} confirma que DAIMO no redefine ni bifurca la descripción del modelo: todos los constructos de MLDCAT-AP se adoptan, y los casos perfilados sólo añaden obligaciones de uso en lugar de nueva semántica. El bloque inferior aísla los constructos puente que MLDCAT-AP no proporciona, que son exactamente las relaciones de gobernanza que motivan el perfil. Leídos en conjunto, ambos bloques muestran que la contribución es relacional y no un vocabulario de modelo competidor.
...
DAIMO adopta la metodología \emph{Linked Open Terms} (LOT)~\cite{lot22} por dos razones. Primera, su orientación industrial: LOT prescribe un flujo ligero con entregables en cada una de sus cuatro fases (especificación de requisitos, implementación, publicación y mantenimiento). Segunda, LOT trata la reutilización como fase de primera clase, no como subproducto, lo que encaja con la regla reuse-first que el problema impone. Las subsecciones siguientes describen el escenario operativo que guió la elicitación de requisitos (§\ref{sec:scenario}), las preguntas de competencia y los requisitos no funcionales resultantes (§\ref{sec:method-req}), y las tres decisiones centrales de diseño que dieron forma a la ontología (§\ref{sec:method-design}).

\subsection{Escenario ilustrativo}\label{sec:scenario}

La especificación de requisitos se ancla en un escenario acotado de predicción de riesgo de inundación, inspirado en intercambios de espacios de datos tipo PIONERA, que posteriormente actúa como caso de estudio (§\ref{sec:eval-usecase}). Un grupo de investigación universitario, actuando como proveedor de modelos, publica un modelo de predicción de riesgo de inundación en un catálogo federado gestionado por un operador de plataforma de espacio de datos. Un consumidor municipal necesita invocar el modelo para apoyar decisiones operativas tras un evento meteorológico extremo, pero no puede hacerlo sin antes descubrirlo bajo una política compatible con su uso público, verificar que satisface un umbral mínimo de calidad sobre un contexto de evaluación común, negociar un acuerdo que autorice sus ejecuciones concretas e invocarlo a través de un endpoint autenticado. Un evaluador de investigación climática compara el modelo con una línea base sobre el mismo contexto de evaluación, y un actor de gobernanza orientado al cumplimiento audita las ejecuciones y archiva la evidencia correspondiente. Las organizaciones, versiones de modelo, endpoints, métricas y fechas usadas en el artículo son ilustrativas: el caso no se presenta como piloto desplegado, sino como demostrador reproducible de adecuación representacional a nivel de artefacto.
...
Los requisitos se elicitaron desde tres fuentes complementarias: el escenario anterior, la especificación y notas de implementación de MLDCAT-AP~3.0.0 y el análisis de términos seleccionados de las extensiones Eclipse EDC empleadas en plataformas de espacios de datos. A partir de ellas se formularon veintitrés preguntas de competencia, organizadas en cinco categorías por actor y etapa del ciclo de vida. La categoría~R (registro y publicación, cinco preguntas) cubre las necesidades del proveedor; la categoría~D (descubrimiento y selección, cuatro preguntas), las del consumidor; la categoría~E (ejecución y auditabilidad, cinco preguntas), las del operador y gobernanza; la categoría~V (evaluación y reproducibilidad, cinco preguntas), las del evaluador; y la categoría~G (puente de gobernanza, cuatro preguntas), las preguntas multi-actor que requieren semántica nativa de DAIMO. El conjunto completo de las veintitrés preguntas en lenguaje natural aparece en el Apéndice~\ref{app:cqs}.

La primera síntesis vincula actores y familias de preguntas antes de introducir cualquier trazabilidad técnica. La Tabla~\ref{tab:actors} asocia cada rol ---proveedor, consumidor, operador de plataforma, evaluador y la perspectiva de gobernanza multi-actor--- con la familia de preguntas de competencia que origina y con la necesidad concreta que hay detrás. Presentar primero a los actores fija quién formula cada requisito y evita que las preguntas de competencia se lean como una lista abstracta desvinculada del escenario. Además deja explícito que la categoría de gobernanza es genuinamente transversal, porque ningún participante aislado puede responderla por sí solo.
...
El segundo paso es trazar cada familia de requisitos hacia el compromiso de modelado que la satisface y hacia la evidencia que después lo demuestra. La Tabla~\ref{tab:req-trace} es, por tanto, la bisagra entre la fase de especificación LOT y la fase de evaluación: para cada requisito operativo nombra el constructo de la ontología que porta la semántica y el artefacto de validación ---consulta de competencia, \emph{shape} SHACL o prueba negativa--- que lo comprueba. Leer la tabla de arriba abajo justifica por qué existe cada bloque principal de la ontología, en lugar de presentar las clases como preferencias de diseño. También anticipa la sección de evaluación, de modo que el lector ya puede ver qué requisito pretende apoyar cada resultado posterior.
...
El desglose de la Tabla~\ref{tab:ontometrics} muestra que las cifras reportadas cuentan sólo sujetos nativos de DAIMO, de modo que las declaraciones de alineamiento que enlazan el perfil con vocabularios externos no inflan los totales. La alta proporción de propiedades funcionales, dominios y rangos refleja un diseño deliberadamente restringido en el que la mayoría de relaciones están tipadas y son de valor único. Los dos ceros de \code{owl:equivalentClass} y de \code{owl:disjointWith} binario son esperables, porque la disjunción se expresa mediante un único axioma nombrado \code{owl:AllDisjointClasses}.
...
La Figura~\ref{fig:ontology-global} ofrece una lectura a nivel de clases del perfil como diagrama ontológico: cada caja muestra el nombre de una clase y, cuando procede, sus atributos principales, y las flechas etiquetadas son las propiedades de objeto que las conectan. Las clases puente aparecen en azul, las clases externas reutilizadas en gris, las clases de evidencia en ámbar y el contexto de evaluación en verde, de modo que se ve de un vistazo qué conceptos introduce DAIMO y cuáles toma de vocabularios consolidados. El diagrama se restringe deliberadamente a las clases y relaciones más importantes de la cadena gobernada ---publicación, despliegue, invocación, autorización, ejecución, evidencia y evaluación--- en lugar de reproducir todos los términos. Debe leerse como el mapa conceptual que el resto de la sección desarrolla grupo a grupo.

\begin{figure*}[tb]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[x=1cm,y=1cm]
  % --- Publicaci\'on y gobernanza ---
  \node[umlclass] (role) at (0,0)
    {\textbf{daimo:ParticipantRole}\nodepart{two}subclases:\\ModelProvider,\\ModelConsumer,\\PlatformOperator,\\Evaluator,\\GovernanceActor};
  ...
  % --- Propiedades de objeto (asociaciones) ---
...
\caption{Vista ontológica global de DAIMO: clases puente propias (azul), clases externas reutilizadas (gris), clases de evidencia (ámbar) y el contexto de evaluación compartido (verde), con sus atributos principales y propiedades de objeto.}
\label{fig:ontology-global}
\end{figure*}

La Figura~\ref{fig:classes} complementa la vista global con el nivel axiomático, de modo que ambas figuras se lean juntas: la anterior muestra qué conecta DAIMO y esta muestra cómo se declara esa conexión en OWL. El panel superior separa las clases puente DAIMO de las clases reutilizadas a las que se adhieren mediante \code{rdfs:subClassOf}, haciendo explícita la superficie de alineamiento. El panel inferior retiene sólo las relaciones operativas principales que articulan el perfil, dejando el listado exhaustivo de términos a la Tabla~\ref{tab:classes}. Separar alineamientos y relaciones en dos paneles mantiene cada lectura despejada.
...
\end{tikzpicture}
\caption{Vista por fases del demostrador multi-participante acotado.}
\label{fig:scenario}
\end{figure}
...
\midrule
Consistencia lógica & HermiT reporta consistencia y cero clases insatisfactibles; OWL-RL materializa 1\,170 tripletas sin \code{owl:Nothing}; la verificación de implicaciones inspecciona catorce clases nativas con cero advertencias. & Evidencia reproducible a nivel de artefacto. La consistencia se complementa con pruebas negativas SHACL para evitar restricciones triviales. \\
Interoperabilidad & El perfil usa RDF/OWL~2~DL, SHACL y SPARQL; reutiliza DCAT, MLDCAT-AP, ODRL, PROV-O, FOAF, SKOS y SPDX mediante IRIs externos y alineamientos axiomáticos. & Evidencia sustancial a nivel de artefacto. La interoperabilidad operacional dependerá de la publicación y consumo efectivos en infraestructuras de terceros. \\
...
Extensibilidad & El núcleo mantiene alcance mínimo y versionado SemVer; las extensiones se orientan a perfiles complementarios, por ejemplo \code{daimo-discovery}, sin absorber lógica de ranking o ejecución. & Evidencia sustancial a nivel de artefacto para evolución controlada. Nuevos conceptos deben justificar que no existen en vocabularios reutilizados. \\
Modularidad & Núcleo, alineamientos, \emph{shapes}, ejemplos, consultas, pruebas negativas y reportes están separados; cada módulo puede ejecutarse o revisarse de forma independiente. & Evidencia reproducible a nivel de artefacto. La independencia modular reduce el riesgo de romper el núcleo al añadir perfiles externos. \\
Documentación & Cada término incluye etiquetas y definiciones SKOS; la documentación suplementaria traza clases, propiedades, CQs, invariantes y comandos de reproducción. & Evidencia sustancial a nivel de artefacto para revisión local y reproducción. La calidad editorial de la documentación pública deberá mantenerse sincronizada con futuras versiones del perfil. \\
\bottomrule
\end{tabular}
\end{table*}

El benchmark se presenta como evidencia acotada de tratabilidad, no como prueba de rendimiento operativo. La Tabla~\ref{tab:scalability} reporta los tiempos y conteos de tripletas obtenidos para grafos de 100 y 1\,000 unidades sintéticas de intercambio, junto con el veredicto de conformidad SHACL en cada caso. Las dos filas pretenden mostrar una tendencia de crecimiento bajo un protocolo controlado, no certificar rendimiento sobre una carga de producción. También aíslan el coste de cada etapa ---parseo, cierre OWL-RL, validación SHACL y respuesta SPARQL--- de modo que el factor dominante quede visible. Las cifras son reproducibles desde la suite de validación publicada bajo la semilla aleatoria fija usada para generar los grafos.
...
Ambas configuraciones se mantienen conformes con SHACL, y la respuesta a consultas permanece muy por debajo de un segundo incluso con 1\,000 unidades, lo que indica que el perfil no introduce un coste patológico de consulta. Como cabía esperar, la validación SHACL es la etapa dominante y crece más rápido que el parseo o el cierre, de modo que es el componente a vigilar cuando los grafos crecen. Una ejecución mayor de 10\,000 unidades se deja deliberadamente como trabajo futuro, porque requeriría ejecución real y no la extrapolación aquí reportada.
...
La ontología, las \emph{shapes} SHACL, el grafo de demostración, las consultas SPARQL, las pruebas negativas, los reportes de validación y las instrucciones de reproducción están organizados en un único paquete en \url{https://github.com/EdmundoMori/Papers-DAIMO} (directorio \code{daimo/}). Para revisión, la instantánea inmutable \url{https://github.com/EdmundoMori/Papers-DAIMO/releases/tag/v0.1.6-swj-submission} constituye la versión congelada de referencia. El namespace canónico \url{https://w3id.org/pionera/daimo} dispone de configuración de redirección preparada (\code{daimo/w3id-redirect/.htaccess}); el registro en \emph{w3id.org} permanece pendiente. Se asignará un DOI de archivo en Zenodo tras la aceptación de la versión final.
...
La Tabla~\ref{tab:future-profiles} hace auditable la decisión de alcance: la columna central muestra que el núcleo actual ya cubre lo que exigen el descubrimiento y la invocación, mientras que la columna derecha nombra el perfil que alojaría cada preocupación adicional. El patrón constante es que las extensiones se añaden como perfiles separados versionados con SemVer y no como nuevas clases núcleo, lo que mantiene estables las catorce clases nativas. Este es el mecanismo por el que DAIMO puede evolucionar hacia reproducibilidad, linaje, monitorización o cumplimiento sin fragmentar su columna vertebral de gobernanza.
...
La columna \emph{Filas} de la Tabla~\ref{tab:cq-trace} es la evidencia reproducible que respalda la afirmación de que las veintitrés preguntas de competencia se responden: cada conteo esperado de filas corresponde a las instancias declaradas en el grafo del demostrador. Como las preguntas recorren toda la cadena gobernada, desde la publicación hasta la evaluación compartida, la tabla funciona además como verificación de cobertura sobre las clases nativas y sus propiedades principales. Un lector puede así reproducir cualquier consulta individual y confirmar que devuelve resultados y que ejercita los constructos que el perfil dice conectar.

\section{Glosario de abreviaturas}\label{app:glossary}

El glosario final recoge las abreviaturas que reaparecen en varias secciones del artículo, desde nombres de vocabularios hasta términos regulatorios y de razonamiento. Se mantiene como una única tabla de consulta para no repetir definiciones en el cuerpo principal y para que la terminología quede alineada entre la versión maestra en español y el inglés técnico. Las entradas se ordenan alfabéticamente para agilizar las búsquedas individuales durante la revisión. Nada en el glosario introduce nuevas afirmaciones; sólo fija las expansiones usadas en el resto del texto.
...
Como la Tabla~\ref{tab:glossary} mezcla nombres de vocabularios, herramientas y referencias regulatorias, también registra el alcance exacto en que se usa cada acrónimo en este artículo. Cuando una abreviatura pudiera ser ambigua fuera del contexto de espacios de datos, la expansión de la tabla es la lectura pretendida a lo largo del texto. Se espera que el glosario permanezca estable entre versiones del perfil, de modo que revisiones posteriores puedan ampliarlo sin redefinir las entradas existentes.

\section{Convenciones de URI}\label{app:uris}

DAIMO utiliza el namespace canónico \url{https://w3id.org/pionera/daimo}, con identificadores de términos en la forma \code{https://w3id.org/pionera/daimo\#<Término>} e IRIs versionadas bajo \code{https://w3id.org/pionera/daimo/<versión>}, enlazadas entre sí mediante \code{owl:priorVersion}. Los módulos satélite son ontologías independientes con IRIs versionadas propias: la capa SHACL bajo \url{https://w3id.org/pionera/daimo/shapes/} y el módulo de alineamientos bajo \url{https://w3id.org/pionera/daimo/align} (este último declarando \code{owl:imports} sobre la ontología base). El grafo de ejemplo utiliza el namespace local \code{https://example.org/daimo-scenario/}, claramente separado del espacio canónico. La negociación de contenido está diseñada para resolver el mismo identificador a HTML, Turtle, RDF/XML, JSON-LD o N-Triples en función del \code{Accept} header. Hasta que el redireccionamiento se registre formalmente, la versión congelada en GitHub \url{https://github.com/EdmundoMori/Papers-DAIMO/releases/tag/v0.1.6-swj-submission} y el repositorio fuente proporcionan los puntos de acceso de referencia para revisión y replicación.
