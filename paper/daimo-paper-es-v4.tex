...
La Tabla~\ref{tab:mldcatap-delta} explicita la relación clase a clase entre DAIMO y MLDCAT-AP. El patrón es intencional: DAIMO adopta las clases de MLDCAT-AP para el plano del activo de IA, añade restricciones de perfil cuando el intercambio gobernado exige completitud operativa, e introduce extensiones sólo para las relaciones puente que MLDCAT-AP no cubre. La versión del paquete validado se mantiene sobre MLDCAT-AP~3.0.0 porque los alineamientos, ejemplos y pruebas SHACL fueron construidos contra esa especificación. MLDCAT-AP~3.1.0, publicada como \emph{Candidate Recommendation} en 2026~\cite{mldcatap31}, debe revisarse antes del envío final para confirmar que la tabla no cambia o, en caso contrario, actualizar el módulo de alineamientos y reejecutar la suite de validación.
...
\midrule
Lenguaje, serialización e IRIs & Núcleo OWL~2~DL en Turtle; serializaciones RDF/XML, JSON-LD y N-Triples preparadas para publicación; namespace canónico \code{https://w3id.org/pionera/daimo}. & El despliegue \code{w3id} y el DOI son pasos FAIR finales; la reproducibilidad local no depende de ellos. \\
...
Almacenamiento y acceso & El paquete RDF puede cargarse en triplestores estándar y exponerse por SPARQL; las consultas publicadas en \code{daimo/queries/queries.md} demuestran el patrón de acceso. & El artículo no afirma un endpoint productivo GraphDB/Fuseki; sólo compatibilidad técnica con ese tipo de despliegue. \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\scriptsize\sf\centering
\caption{Trazabilidad resumida CQ $\to$ clases $\to$ propiedades $\to$ resultado. Las consultas completas se publican en \code{daimo/queries/queries.md}; esta tabla permite inspeccionar dentro del PDF qué fragmento semántico ejerce cada CQ.\label{tab:cq-trace}}
...
\begin{table*}[t]
\scriptsize\sf\centering
\caption{Cobertura de requisitos no funcionales de DAIMO. La columna de estado distingue cumplimiento demostrado en el artefacto de pendientes operativos para el paquete público final.\label{tab:nfr-coverage}}
...
Interoperabilidad & El perfil usa RDF/OWL~2~DL, SHACL y SPARQL; reutiliza DCAT, MLDCAT-AP, ODRL, PROV-O, FOAF, SKOS y SPDX mediante IRIs externos y alineamientos axiomáticos. & Cumplimiento fuerte en el artefacto. Persisten pendientes operativos de publicación FAIR: \code{w3id}, DOI Zenodo y WIDOCO final. \\
Escalabilidad & \code{scalability\_benchmark.py} genera grafos sintéticos conformes de 100 y 1\,000 unidades de intercambio; ambos conforman SHACL y responden las consultas de control. & Cumplimiento medido y acotado. SHACL es el coste dominante; no se afirma rendimiento productivo ni resultado de 10\,000 unidades sin ejecutarlo. \\
...
Modularidad & Núcleo, alineamientos, \emph{shapes}, ejemplos, consultas, pruebas negativas y reportes están separados; cada módulo puede ejecutarse o revisarse de forma independiente. & Cumplimiento fuerte. La independencia modular reduce el riesgo de romper el núcleo al añadir perfiles externos. \\
Documentación & Cada término incluye etiquetas y definiciones SKOS; \code{ONTOLOGY-REFERENCE.md}, \code{VALIDATION-MATRIX.md} y \code{REPRODUCIBILITY.md} trazan clases, propiedades, CQs, invariantes y comandos. & Cumplimiento fuerte localmente. Antes del envío debe regenerarse la documentación pública WIDOCO y cerrar metadatos ORCID/DOI. \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\small\sf\centering
\caption{Benchmark sintético de escalabilidad acotada. Cada unidad contiene una oferta, modelo, política ODRL, despliegue, servicio, contrato de I/O, autorización, ejecución, artefacto derivado, evidencia de auditoría, evaluación y bundle de procedencia. Reporte completo: \code{reports/scalability-benchmark.md}.\label{tab:scalability}}
...
La tercera línea concierne a las \emph{condiciones de reproducibilidad y validación externa}. Los resultados numéricos reportados dependen de un perfil de razonamiento concreto: el modo \code{inference=rdfs} en pyshacl, necesario para que las subpropiedades implicadas se materialicen antes de evaluar los \emph{shapes} de conformidad, y el cierre OWL-RL materializado antes de las consultas de competencia. Esta dependencia está documentada pero es una condición que un consumidor debe conocer para reproducir los resultados exactos. La versión actual reporta validación ejecutable; la versión de envío debe incorporar una validación externa ligera con tres perfiles, diseñada como revisión cualitativa estructurada y no como experimento estadístico. Para evitar que el juicio experto sea una revisión informal, los materiales deben separarse por audiencia: glosario normalizado, preguntas de competencia, diagrama conceptual y escenario anónimo para expertos de dominio; extractos OWL, alineamientos y \emph{shapes} para el perfil Semantic Web. Las preguntas dirigidas a expertos no sustituyen a las preguntas de competencia de LOT: las CQs se verifican mediante SPARQL sobre el grafo de demostración, mientras que la revisión experta juzga suficiencia, claridad, alineación con prácticas de espacio de datos y utilidad para MLOps o cumplimiento. La aceptación no exige unanimidad ni una cobertura nominal del 100\%; exige que toda observación quede trazada como cambio aceptado, cambio rechazado con justificación, fuera de alcance o trabajo futuro, junto con su impacto sobre clases, propiedades, CQs o restricciones. La Tabla~\ref{tab:expert-validation} fija el protocolo mínimo y añade cumplimiento como perfil opcional recomendado cuando el artículo mantenga referencias explícitas al AI Act, sesgo o responsabilidad.

\begin{table*}[t]
\small\sf\centering
\caption{Protocolo mínimo de validación externa para cerrar el envío SWJ. La tabla no reporta resultados ya obtenidos; fija la evidencia que debe incorporarse antes de la sumisión.\label{tab:expert-validation}}
\begin{tabular}{@{}p{3.4cm}p{4.2cm}p{4.8cm}p{2.6cm}@{}}
\toprule
\textbf{Perfil} & \textbf{Objeto de revisión} & \textbf{Pregunta principal} & \textbf{Salida esperada} \\
\midrule
Ontologías / Semantic Web & Modularidad, alineamientos, CQs, SHACL & ¿Los compromisos ontológicos son suficientes y no redundantes frente a vocabularios reutilizados? & Lista de ajustes de modelado y documentación. \\
Data spaces / EDC & Oferta, autorización, contexto de participante, DSP/ODRL & ¿La cadena publicación-negociación-invocación-auditoría representa un intercambio gobernado realista? & Observaciones sobre integración con conectores y contratos. \\
ML / MLOps & Modelo, ejecución, evaluación, reproducibilidad & ¿Las entidades de ejecución y comparación son útiles para gestión práctica de modelos? & Observaciones sobre evaluación, latencia, métricas y límites. \\
Gobernanza / cumplimiento (opcional) & Políticas, evidencia, roles y trazabilidad regulatoria & ¿La evidencia DAIMO es suficiente para sostener auditorías de uso, responsabilidad y riesgo sin prometer cumplimiento legal automático? & Observaciones sobre alcance regulatorio, sesgo, explicabilidad y límites jurídicos. \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
Namespace persistente & \url{https://w3id.org/pionera/daimo}; configuración de negociación de contenido preparada en \code{w3id-redirect/.htaccess}. Estado: pendiente de merge en \code{perma-id/w3id.org}. \\
...
Repositorio público & Paquete de código y documentación preparado; la URL definitiva del repositorio público debe fijarse antes del envío final siguiendo \code{DEPLOYMENT.md}. \\
Identificación de autoría & ORCID(s) de los autores pendientes de confirmar e incorporar en la versión de sumisión y en \code{CITATION.cff}. \\
Archivado versionado (DOI) & Release versionada preparada para archivado en Zenodo; el DOI final debe incorporarse en la versión de sumisión. \\
...
El flujo de reproducibilidad no requiere el despliegue previo de la URL persistente: cualquier usuario con Python~3.12 y las dependencias declaradas (\code{rdflib}, \code{pyshacl}, \code{owlrl}, \code{owlready2}) puede ejecutar los cinco scripts localmente y reproducir los reportes de \code{reports/}. La URL persistente y el DOI son requisitos FAIR adicionales que complementan pero no condicionan la reproducibilidad. La cabecera de la ontología declara \code{dct:conformsTo} hacia OWL~2 y utiliza \code{rdfs:seeAlso} para enlazar los tres módulos y la documentación HTML, de modo que un consumidor puede recorrer la cadena identificador-documento sin cargar el paquete completo.

La sostenibilidad del artefacto se estructura según la Fase~4 de LOT. El fichero \code{CONTRIBUTING.md} documenta el flujo de triaje de \emph{issues}, el ciclo de pull requests y la política de versionado según SemVer: las versiones \emph{patch} se reservan para correcciones de definición textual o de documentación, las \emph{minor} para nuevas clases o propiedades compatibles y las \emph{major} para cambios que alteren alineamientos o invariantes. Cada cambio no trivial debe declarar qué actividades LOT se reabren de forma acotada y qué impacto tiene sobre preguntas de competencia, \emph{shapes} SHACL, axiomas de alineamiento, grafos ya instanciados e interoperabilidad entre nodos del espacio de datos. Una integración continua pública es un cierre de ingeniería deseable para ejecutar automáticamente sintaxis RDF, HermiT/OWL-RL, SHACL, OOPS! y pruebas negativas en cada cambio; no se presenta como evidencia adicional mientras no esté desplegada. La política de deprecación impone al menos una versión \emph{minor} de coexistencia antes de retirar un término, con \code{owl:deprecated} y \code{skos:closeMatch} hacia el término de reemplazo.
