...
La especificación de requisitos se ancla en el espacio de datos PIONERA, una iniciativa española sobre la mejora de la interoperabilidad en espacios de datos mediante inteligencia artificial, que posteriormente actúa como caso de estudio (§\ref{sec:eval-usecase}). En PIONERA, un conector de Empresa actúa como proveedor de modelos y un conector de Ayuntamiento actúa como consumidor municipal, coordinados por un operador de plataforma que gestiona el catálogo federado y la negociación entre conectores. El catálogo expone dos familias de modelos que motivan los requisitos: los modelos de Movilidad, que predicen tiempo de viaje y retraso del transporte público a partir de características de tramo tipo GTFS, y los modelos FLARES, que extraen eventos y clasifican fiabilidad sobre texto en español. El consumidor municipal necesita invocar un modelo de Movilidad de predicción de retraso para apoyar decisiones operativas, pero no puede hacerlo sin antes descubrirlo bajo una política compatible con su uso público, verificar que satisface un umbral mínimo de calidad sobre un contexto de evaluación común, negociar un acuerdo que autorice sus ejecuciones concretas e invocarlo a través de un endpoint autenticado. Un servicio de benchmarking compara el modelo con una línea base sobre el mismo contexto de evaluación, y un actor de gobernanza audita las ejecuciones y archiva la evidencia correspondiente. Los nombres, modelos, endpoints, métricas y fechas del escenario son sintéticos por diseño: el caso no se presenta como piloto desplegado, sino como demostrador reproducible de adecuación representacional.
...
\begin{tikzpicture}[node distance=0.23cm]
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{1. Publicaci\'on} \hfill Conector de Empresa\\
    \code{AIAssetOffering}; \code{dcat:Catalog}; \code{odrl:Offer}.};

  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    align=left, fill=white, below=of s1] (s2)
    {\textbf{2. Descubrimiento y selecci\'on} \hfill Conector de Ayuntamiento\\
  ...
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{4. Ejecuci\'on autorizada} \hfill Conector de Ayuntamiento\\
  ...
  \node[phasebox, text width=0.86\columnwidth, minimum width=0.92\columnwidth,
    ...
    {\textbf{6. Evaluaci\'on compartida} \hfill Servicio de benchmarking\\
...
\midrule
Consistencia lógica & HermiT reporta consistencia y cero clases insatisfactibles; OWL-RL materializa 1\,170 tripletas sin \code{owl:Nothing}; la verificación de implicaciones inspecciona catorce clases nativas con cero advertencias. & Evidencia fuerte a nivel de artefacto. La consistencia se complementa con pruebas negativas SHACL para evitar restricciones triviales. \\
Interoperabilidad & El perfil usa RDF/OWL~2~DL, SHACL y SPARQL; reutiliza DCAT, MLDCAT-AP, ODRL, PROV-O, FOAF, SKOS y SPDX mediante IRIs externos y alineamientos axiomáticos. & Evidencia fuerte a nivel de artefacto. La interoperabilidad operacional dependerá de la publicación y consumo efectivos en infraestructuras de terceros. \\
...
Extensibilidad & El núcleo mantiene alcance mínimo y versionado SemVer; las extensiones se orientan a perfiles complementarios, por ejemplo \code{daimo-discovery}, sin absorber lógica de ranking o ejecución. & Evidencia fuerte a nivel de artefacto para evolución controlada. Nuevos conceptos deben justificar que no existen en vocabularios reutilizados. \\
Modularidad & Núcleo, alineamientos, \emph{shapes}, ejemplos, consultas, pruebas negativas y reportes están separados; cada módulo puede ejecutarse o revisarse de forma independiente. & Evidencia fuerte a nivel de artefacto. La independencia modular reduce el riesgo de romper el núcleo al añadir perfiles externos. \\
Documentación & Cada término incluye etiquetas y definiciones SKOS; la documentación suplementaria traza clases, propiedades, CQs, invariantes y comandos de reproducción. & Evidencia fuerte a nivel de artefacto para revisión local y reproducción. La calidad editorial de la documentación pública deberá mantenerse sincronizada con futuras versiones del perfil. \\
\bottomrule
\end{tabular}
\end{table*}
